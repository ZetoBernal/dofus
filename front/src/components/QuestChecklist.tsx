...
function QuestRow({
  ...
  onToggle: () => void;
  override?: MissionOverrideData;
}) {
  const { language } = useLanguage();
  const [expanded, setExpanded] = useState(false);

  const enEspanol = language === "es";
  const pasos = override?.pasos ?? [];
  const tieneTraduccion = enEspanol && (!!override?.nombreEs || pasos.length > 0);
  const nombreMostrado = enEspanol && override?.nombreEs ? override.nombreEs : quest.mision;
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-2 pl-7">
            ...
            {enEspanol && pasos.length > 0 && (
              <button
                type="button"
                onClick={() => setExpanded((e) => !e)}
                className="inline-flex items-center gap-1 rounded-full bg-amber-50 pl-2 pr-2 py-0.5 text-[11px] font-medium text-amber-700 hover:bg-amber-100 dark:bg-amber-950 dark:text-amber-400 dark:hover:bg-amber-900 cursor-pointer"
              >
                {expanded ? "Ocultar pasos ▲" : `Ver ${pasos.length} paso${pasos.length > 1 ? "s" : ""} ▼`}
              </button>
            )}
          </div>

          {enEspanol && expanded && pasos.length > 0 && (
            <ol className="mt-3 ml-7 flex flex-col gap-3 border-l-2 border-amber-200 dark:border-amber-900 pl-4">
              {pasos.map((p, i) => (
                <li key={i} className="text-sm text-zinc-600 dark:text-zinc-300">
                  <p>{p.texto}</p>
                  {p.imagen && (
                    // eslint-disable-next-line @next/next/no-img-element -- imagen subida por el admin, tamaño variable
                    <img
                      src={p.imagen}
                      alt=""
                      className="mt-2 max-w-full rounded-lg border border-zinc-200 dark:border-zinc-800"
                    />
                  )}
                </li>
              ))}
...
function ZoneSection({
  ...
  overrides: Record<string, MissionOverrideData>;
}) {
