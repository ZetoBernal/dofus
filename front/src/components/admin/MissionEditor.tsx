...
// Todo pega directo al backend vía /backend/* (rewrite en next.config.ts),
// same-origin desde el navegador, así la cookie de sesión viaja sola.
async function saveMission(mision: string, data: { nombreEs: string | null; pasos: StepInput[] }) {
  const res = await fetch(`/backend/api/mission-overrides/${encodeURIComponent(mision)}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(data),
  });
  if (res.status === 401) throw new SessionExpiredError();
  if (!res.ok) throw new Error("No se pudo guardar la traducción.");
}

async function deleteMission(mision: string) {
  const res = await fetch(`/backend/api/mission-overrides/${encodeURIComponent(mision)}`, {
    method: "DELETE",
  });
  if (res.status === 401) throw new SessionExpiredError();
  if (!res.ok) throw new Error("No se pudo borrar la traducción.");
}

async function uploadStepImage(formData: FormData): Promise<{ path: string } | { error: string }> {
  const res = await fetch("/backend/api/uploads/steps", { method: "POST", body: formData });
  if (res.status === 401) throw new SessionExpiredError();
  const body = await res.json().catch(() => null);
  if (!res.ok) return { error: body?.message ?? "No se pudo subir la imagen." };
  return body;
}
...
export function MissionEditor({
  ...
  async function handleFile(key: string, file: File) {
    ...
    try {
      ...
      if (err instanceof SessionExpiredError) return router.push("/admin/login");
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (err instanceof SessionExpiredError) return router.push("/admin/login");
      setMessage("No se pudo guardar la traducción.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (err instanceof SessionExpiredError) return router.push("/admin/login");
      setMessage("No se pudo borrar la traducción.");
    } finally {
      setSaving(false);
    }
  }
